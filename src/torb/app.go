...
func main() {
	dsn := fmt.Sprintf("%s:%s@tcp(%s:%s)/%s?parseTime=true&charset=utf8mb4",
		os.Getenv("ISUCON_DB_USER"), os.Getenv("ISUCON_DB_PASSWORD"),
		os.Getenv("ISUCON_DB_HOST"), os.Getenv("ISUCON_DB_PORT"),
		os.Getenv("ISUCON_DB"),
	)
	...
	e.GET("/initialize", func(c echo.Context) error {
		_, err := db.Exec("DELETE FROM administrators WHERE id >= 102")
		if err != nil {
			return err
		}
		_, err = db.Exec("DELETE FROM events WHERE id >= 19")
		if err != nil {
			return err
		}
		_, err = db.Exec("DELETE FROM reservations WHERE id >= 191808")
		if err != nil {
			return err
		}
		_, err = db.Exec("DELETE FROM sheets WHERE id >= 1001")
		if err != nil {
			return err
		}
		_, err = db.Exec("DELETE FROM users WHERE id >= 5001")
		if err != nil {
			return err
		}
